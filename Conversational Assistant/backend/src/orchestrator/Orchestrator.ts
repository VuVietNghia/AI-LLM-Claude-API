...
import { ChatMessage, ToolDefinition } from '../llm/types.js';
...
export class Orchestrator {
  ...
  private setupTools() {
    const aiExecutor = async (prompt: string) => {
      ...
      return await this.llmClient.chat(msgs);
    };

    this.toolRegistry.register(new CalculatorTool(aiExecutor));
  ...
  async process(messages: ChatMessage[], onEvent: (event: OrchestratorEvent) => void) {
    ...
    while (steps < MAX_STEPS) {
      ...
      const stream = this.llmClient.chatStream(currentMessages, tools);
      ...
      if (toolCallsToProcess.length > 0) {
        ...
        for (const tc of toolCallsToProcess) {
          ...
          try {
             ...
             console.error(`Lỗi parse args cho tool ${fnName}`, e);
          }
