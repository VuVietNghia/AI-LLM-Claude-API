  ...
  async execute(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    try {
      ...
      if (ext && !ALLOWED_EXTENSIONS.includes(ext)) {
        return { success: false, data: '', error: { code: 'INVALID_EXTENSION', message: `Ext ${ext} not allowed.`, userFriendly: `Không được phép đọc file định dạng ${ext}.` } };
      }

      const stat = await fs.stat(absolutePath);
      if (stat.size > config.limits.fileReadLimitBytes) {
         return { success: false, data: '', error: { code: 'FILE_TOO_LARGE', message: 'File too large', userFriendly: `File quá lớn.` } };
      }

      const content = await fs.readFile(absolutePath, 'utf-8');
      return { success: true, data: content };
    } catch (err: any) {
      return { success: false, data: '', error: { code: 'READ_ERROR', message: err.message, userFriendly: `Không thể đọc file: ${err.message}` } };
    }
  }
}
...
export class WriteFileTool implements ITool {
  ...
  async execute(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    try {
      ...
      if (ext && !ALLOWED_EXTENSIONS.includes(ext)) {
         return { success: false, data: '', error: { code: 'INVALID_EXTENSION', message: `Ext ${ext} not allowed.`, userFriendly: `Không được phép ghi file định dạng ${ext}.` } };
      }
